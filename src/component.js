    ...
    return (
      <article className={classNames(style.article, className)}>
        ...
        <div className={style.article__content}>
          <Text bind={`steps[${index}].heading`} className={classNames(style.article__title, this.ui('ui-heading'))} tagName="h2" />
          {_.get('body')(modifier) && (
            <Text bind={`steps[${index}].description`} className={classNames(style.article__text, this.ui('ui-body'))} tagName="p" />
          )}
        </div>
      </article>
    )
