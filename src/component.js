...
class Block extends React.Component {
  ...
  collectionItem = planIndex => ({index, children, className}) => {
    ...
    return (
      <li className={classNames(style['plan__list-item'], className)}>
        ...
        <Text
          tagName="span"
          className={classNames(style['plan__list-item-text'], this.ui('ui-body'), this.ui('ui-text-center'))}
  ...
  planHeader = (itemNumber) => {
    ...
    return [
      <Text
        tagName="h2"
        className={classNames(style.plan__title, this.ui('ui-heading'), this.ui('ui-text-center'))}
      ...
      this.getModifierValue('plan-price') && (
        <p className={style.plan__price}>
          <Text
            tagName="strong"
            className={classNames(style['plan__price-count'], this.ui('ui-heading-lg'), this.ui('ui-text-center'))}
            bind={`plan-price-count-${itemNumber}`}
          />
          {this.getModifierValue('plan-additional-info') && (
            <Text
              tagName="span"
              className={classNames(style['plan__price-text'], 'caption', this.ui('ui-text-center'))}
