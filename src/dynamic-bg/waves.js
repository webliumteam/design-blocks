...
export class Wave extends React.Component {
  ...
  componentDidMount() {
    // console.log(this.props.canvasContainer)
  ...
  render() {
    ...
    return <div ref={el => (this.el = el)} {...rest} className={css.canvas} />
  }
}
...
class WaveScene {
  constructor(el, opts = {}) {
    ...
    this.elHalfY = this.el.offsetHeight / 4
  ...
  init() {
    let i = 0
    for (let ix = 0; ix < this.opts.amountX; ix++) {
      for (let iy = 0; iy < this.opts.amountY; iy++) {
        const particle = (this.particles[i++] = new Sprite(this.material))
        particle.position.x =
          ix * this.opts.separation - (this.opts.amountX * this.opts.separation) / 2
        particle.position.z =
          iy * this.opts.separation - (this.opts.amountY * this.opts.separation) / 2
    ...
    this.registerListener(window, 'mousemove', evt => this.onDocumentMouseMove(evt))
    this.registerListener(window, 'touchstart', evt => this.onDocumentTouch(evt))
    this.registerListener(window, 'touchmove', evt => this.onDocumentTouch(evt))
